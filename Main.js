...
export default class Main extends React.Component {
  ...
  _captureSectionChooseRef = (ref) => { this._sectionChooseListRef = ref; };

  _sectionListRef;
  _captureSectionRef = (ref) => { this._sectionListRef = ref; };
    ...
    return (
      <View style={[styles.itemMessageView, { borderTopWidth: index == 0 ? 0 : 1 }]}>
        <View style={styles.itemMessageViewItwmOne}>
          <Text style={{ fontSize: 12, paddingLeft: 14, paddingRight: 14, color: '#999999' }}>{item.title}</Text>
        </View>
        <View style={{ flex: 2, flexDirection: 'row', backgroundColor: _judeBackColor(item.okone, item.oktwo) }}>
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', borderColor: '#CCCCD2', /*borderLeftWidth: 1, */borderRightWidth: 1 }}>
            <Text style={{ fontSize: 12, color: item.okone ? '#1FB113' : '#FF5A5A' }}>{_jude(item.okone)}</Text>
          </View>
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            <Text style={{ fontSize: 12, color: item.okone ? '#1FB113' : '#FF5A5A' }}>{_jude(item.oktwo)}</Text>
          </View>
        </View>
      </View>
    );
  ...
  _renderSectionHeader = ({ section }) => (
    <View style={styles.sectionHeader}>
      ...
      <View style={{ marginLeft: 14, overflow: "hidden", width: 8, height: 8, borderRadius: 8, backgroundColor: '#666666', }} />
